...
#dataset
train_dataset = DoubleStage(config['dataset']['train_path'], tokenizer)
train_data_loader = DataLoader(train_dataset, batch_size=training_args['batch_size'], shuffle=True)
val_dataset = DoubleStage(config['dataset']['val_path'], tokenizer)
...
for epoch in range(training_args['epochs']):
    ...
    for batch in tqdm(train_data_loader):
        ...
        input_ids = input_ids.squeeze(1)  # Shape becomes [16, 128]
        ...
        generated_ids = model.generate(input_ids, repetition_penalty=2.0,max_length=config['dataset']['max_story_length'], attention_mask=attention_mask,pad_token_id=tokenizer.pad_token_id,num_return_sequences=1)
    ...
    if val_avg_loss < lowest_val_loss:
        model.save_pretrained(config['model']['save_path']+'double_stage')
